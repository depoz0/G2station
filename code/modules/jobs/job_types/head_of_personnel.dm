/datum/job/head_of_personnel
	...
	description = "Изменяйте доступ по ID картам, управляйте отделом обслуживания, \
		защищать сеть, возглавлять станцию, когда капитан погибает."
...
/datum/job/head_of_personnel/get_captaincy_announcement(mob/living/captain)
	return "В связи с нехваткой персонала на борту назначен новый исполняющий обязанности капитана [captain.real_name]!"
...
/obj/item/paper/fluff/ids_for_dummies
	name = "Заметки: Новые ID и Вы"
	desc = "Похоже, что это оставлено последним начальником отдела кадров, работавшим на этой станции. Здесь поясняется некоторая информация о новых ID."
	default_raw_text = {"
<h1>Руководство для чайников по ID</h1>
<h2>Основы</h2>
