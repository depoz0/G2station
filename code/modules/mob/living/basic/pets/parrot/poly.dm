...
/mob/living/basic/parrot/poly
	name = "Поли"
	desc = "Попугай Поли. Эксперт по советам."
...
/mob/living/basic/parrot/poly/get_static_list_of_phrases() // there's only one poly, so there should only be one ongoing list of phrases. i guess
	...
	phrases_to_return += list(
		":e Проверьте кристалл, бля!",
		":e О БОЖЕ, ОН ВОТ-ВОТ РАЗРУШИТСЯ, ВЫЗЫВАЙТЕ ШАТТЛ",
		":e КТО ЗАБРАЛ ЭТИ ЧЕРТОВЫ КОСТЮМЫ?",
		":e Подключите солнечные батареи, ленивые бездельники!",
		"Поли хочет печеньку!",
	)
