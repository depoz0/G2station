...
/obj/structure/sink/attack_hand(mob/living/user, list/modifiers)
	...
	user.visible_message(span_notice("[user] умывает [user.p_their()] [washing_face ? "лицо" : "руки"] используя [rusrep(src.name, 4)]."), \
						span_notice("Вы помыли [washing_face ? "лицо" : "руки"] используя [rusrep(src.name, 4)]."))
...
/obj/structure/curtain/wirecutter_act(mob/living/user, obj/item/I)
	...
	user.visible_message(span_warning("[user] разрезает на части [src.name]."),
		span_notice("Вы начинаете разрезать на части [src.name]."), span_hear("Вы слышите звук резания."))
	if(I.use_tool(src, user, 50, volume=100) && !anchored)
		to_chat(user, span_notice("Вы разрезали [src]."))
		deconstruct()
