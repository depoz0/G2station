/obj/machinery/camera
	...
	desc = "Она используется для мониторинга помещений."
...
/obj/machinery/camera/examine(mob/user)
	...
	if(!status)
		...
		if(!panel_open && powered())
			. += span_notice("Чтобы включить его снова, нужно открыть панель обслуживания с помощью <b>отвертки</b>.")
	if(panel_open)
		. += span_info("Его панель обслуживания в настоящее время открыта.")
		if(!status && powered())
