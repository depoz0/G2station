...
/datum/storage/proc/handle_extra_examination(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("[source] может держать: [can_hold_description]")
...
/datum/storage/proc/item_insertion_feedback(mob/user, obj/item/thing, override = FALSE)
	...
	if(!silent_for_user)
		to_chat(user, span_notice("Вы помещаете [thing.name] [(insert_preposition == null)? "на" : "[insert_preposition]"] [rusrep(parent.name, 4)]."))

	for(var/mob/viewing in oviewers(user))
		if(in_range(user, viewing) || (thing?.w_class >= WEIGHT_CLASS_NORMAL))
			viewing.show_message(span_notice("[user] помещает [thing.name] [(insert_preposition == null)? "на" : "[insert_preposition]"] [rusrep(parent.name, 4)]."), MSG_VISUAL)
...
/datum/storage/proc/dump_content_at(atom/dest_object, mob/user)
	...
	if(dest_object.atom_storage)
		to_chat(user, span_notice("Вы выгружаете содержимое [parent] в [dest_object]."))
	...
	to_chat(user, span_notice("Вы начинаете выгружать содержимое [parent] на [dest_object]..."))
...
/datum/storage/proc/toggle_collection_mode(mob/user)
	...
	switch(collection_mode)
		if(COLLECT_SAME)
			parent.balloon_alert(user, "теперь будет подбирать только один тип")
		if(COLLECT_EVERYTHING)
			parent.balloon_alert(user, "теперь будет подбирать все")
		if(COLLECT_ONE)
			parent.balloon_alert(user, "теперь будет подбирать по одному")
