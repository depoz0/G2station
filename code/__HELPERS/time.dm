...
/proc/DisplayTimeText(time_value, round_seconds_to = 0.1)
	...
	if(!second)
		return "прямо сейчас"
	if(second < 60)
		return "[second] [runam(second, "секунду", "секунды", "секунд")]"
	var/minute = FLOOR(second / 60, 1)
	second = FLOOR(MODULUS(second, 60), round_seconds_to)
	var/secondT
	if(second)
		secondT = " и [second] [runam(second, "секунду", "секунды", "секунд")]"
	if(minute < 60)
		return "[minute] [runam(minute, "минуту", "минуты", "минут")]"
	var/hour = FLOOR(minute / 60, 1)
	minute = MODULUS(minute, 60)
	var/minuteT
	if(minute)
		minuteT = " и [minute] [runam(minute, "минуту", "минуты", "минут")]"
	if(hour < 24)
		return "[hour] [runam(hour, "час", "часа", "часов")]"
	var/day = FLOOR(hour / 24, 1)
	hour = MODULUS(hour, 24)
	var/hourT
	if(hour)
		hourT = " и [hour] [runam(hour, "час", "часа", "часов")]"
	return "[day] [runam(day, "день", "дня", "дней")][hourT][minuteT][secondT]"
